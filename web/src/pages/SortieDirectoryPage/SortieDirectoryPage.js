import { createContext, useEffect, useState } from 'react'

import {
  Tabs,
  Avatar,
  Typography,
  Tab,
  Box,
  Stack,
  Modal,
  TextField,
  Button,
  ButtonGroup,
} from '@mui/material'
import {
  startOfToday,
  startOfTomorrow,
  format,
  endOfWeek,
  startOfWeek,
} from 'date-fns'
import PropTypes from 'prop-types'

import { useAuth } from '@redwoodjs/auth'

import DebriefFormCell from 'src/components/DebriefFormCell/DebriefFormCell'

import DebriefSortieArchive from '../../components/DebriefSortieArchiveCell'
import DebriefSortieCardCell from '../../components/DebriefSortieCardCell/DebriefSortieCardCell'

export const DebriefFormContext = createContext()
export const closeHandler = createContext()

const SortieDirectoryPage = () => {
  const [inProgressPressed, setInProgressPressed] = useState(true)
  const [completedPressed, setCompletedPressed] = useState(false)
  const [value, setValue] = useState(0)
  const [topTab, setTopTab] = useState(0)
  const [open, setOpen] = useState(false)
  const [curSortie, setCurSortie] = useState(null)
  const [edit, setEdit] = useState(false)
  const [begin, setBegin] = useState(startOfWeek(new Date(Date.now())))
  const [end, setEnd] = useState(endOfWeek(new Date(Date.now())))
  const start = new Date(startOfToday()).toISOString()

  function TabPanel(props) {
    const { children, value, index, ...other } = props
    return (
      <div
        role="tabpanel"
        hidden={value !== index}
        id={`simple-tabpanel-${index}`}
        aria-labelledby={`simple-tab-${index}`}
        {...other}
      >
        {value === index && (
          <Box sx={{ p: 3 }}>
            <Typography component={'div'}>{children}</Typography>
          </Box>
        )}
      </div>
    )
  }

  TabPanel.propTypes = {
    children: PropTypes.node,
    index: PropTypes.number.isRequired,
    value: PropTypes.number.isRequired,
  }

  function a11yProps(index) {
    return {
      id: `simple-tab-${index}`,
      'aria-controls': `simple-tabpanel-${index}`,
    }
  }
  const handleTopChange = (event, newValue) => {
    setTopTab(newValue)
  }

  const handleClose = () => {
    setCurSortie(null)
    setOpen(false)
  }

  useEffect(() => {
    if (curSortie) {
      setOpen(true)
    }
  }, [curSortie])

  const { currentUser } = useAuth()

  const activeButtonStyle = {
    '&.Mui-disabled': {
      backgroundColor: '#39f',
      color: '#FFF',
      boxShadow: 'none',
    },
    boxShadow: 'none',
    shadow: 'none',
  }
  const inActiveButtonStyle = {
    backgroundColor: '#bdbdbd',
    color: 'rgba(0, 0, 0, 0.2)',
    boxShadow: 'none',
    shadow: 'none',
    '&:hover': {
      backgroundColor: '#e0e0e0',
      color: 'rgba(0, 0, 0, 0.2)',
    },
  }

  return (
    <Box sx={{ paddingTop: '100px' }}>
      <Box
        sx={{
          display: 'flex',
          flexDirection: 'row',
          alignItems: 'center',
          justifyContent: 'space-between',
        }}
      >
        <Box
          sx={{
            display: 'flex',
            flexDirection: 'row',
            alignItems: 'center',
          }}
        >
          <Avatar
            alt="Unit patch."
            src="data:image/png;base64,"
            sx={{
              margin: '20px',
              top: 'auto',
              width: '22%',
              height: '22%',
            }}
          />
          <Stack>
            <Typography variant="h4" sx={{ textAlign: 'left', margin: '0px' }}>
              Sortie Directory
            </Typography>
          </Stack>
        </Box>
      </Box>
      <Box sx={{ width: '100%' }}>
        <Tabs
          value={topTab}
          onChange={handleTopChange}
          aria-label="Sortie Directory Archive Tab"
          sx={{ borderBottom: 1, borderColor: 'divider' }}
        >
          <Tab label="Daily" {...a11yProps(0)} />
          <Tab label="Archive" {...a11yProps(1)} />
        </Tabs>

        <TabPanel value={topTab} index={0}>
          <Modal
            style={{
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
            }}
            open={open}
            aria-labelledby="modal-modal-title"
            aria-describedby="modal-modal-description"
          >
            <closeHandler.Provider value={{ handleClose }}>
              {curSortie ? (
                <DebriefFormCell id={curSortie} edit={edit} />
              ) : null}
            </closeHandler.Provider>
          </Modal>

          <Box
            sx={{
              display: 'flex',
              justifyContent: 'space-between',
              marginBottom: '24px',
            }}
          >
            <Typography variant="h5">
              {format(new Date(Date.now()), 'LLLL dd, yyyy')}
            </Typography>

            <ButtonGroup disableElevation variant="contained">
              <Button
                disabled={inProgressPressed}
                onClick={() => {
                  setInProgressPressed(true)
                  setCompletedPressed(false)
                  setValue(0)
                }}
                sx={inProgressPressed ? activeButtonStyle : inActiveButtonStyle}
              >
                In Progress
              </Button>
              <Button
                disabled={completedPressed}
                onClick={() => {
                  setInProgressPressed(false)
                  setCompletedPressed(true)
                  setValue(1)
                }}
                sx={completedPressed ? activeButtonStyle : inActiveButtonStyle}
              >
                Completed
              </Button>
            </ButtonGroup>
          </Box>

          <DebriefFormContext.Provider
            value={{ curSortie, setCurSortie, value, edit, setEdit }}
          >
            <DebriefSortieCardCell
              start={start}
              end={startOfTomorrow()}
              unit_id={currentUser?.shop.unit_id}
            />
          </DebriefFormContext.Provider>
        </TabPanel>

        <TabPanel value={topTab} index={1}>
          <Box>
            <Modal
              style={{
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
              }}
              open={open}
              aria-labelledby="modal-modal-title"
              aria-describedby="modal-modal-description"
            >
              <closeHandler.Provider value={{ handleClose }}>
                {curSortie ? (
                  <DebriefFormCell
                    id={curSortie}
                    edit={edit}
                    close={handleClose}
                  />
                ) : null}
              </closeHandler.Provider>
            </Modal>
            <Box>
              <Stack direction="row" justifyContent="space-around">
                <Box sx={{ minWidth: '500px' }}>
                  <TextField
                    type="date"
                    label="Start Date"
                    sx={{ width: '100%' }}
                    value={format(begin, 'yyyy-MM-dd')}
                    onChange={(e) => setBegin(new Date(e.target.value))}
                  >
                    Start Date
                  </TextField>
                </Box>
                <Box sx={{ minWidth: '500px' }}>
                  <TextField
                    type="date"
                    label="End Date"
                    sx={{ width: '100%' }}
                    value={format(end, 'yyyy-MM-dd')}
                    onChange={(e) => {
                      setEnd(new Date(e.target.value))
                    }}
                  >
                    End Date
                  </TextField>
                </Box>
              </Stack>
            </Box>
            <Box sx={{ paddingTop: '2%' }}>
              <DebriefFormContext.Provider value={{ setCurSortie, setEdit }}>
                <DebriefSortieArchive start={begin} end={end} />
              </DebriefFormContext.Provider>
            </Box>
          </Box>
        </TabPanel>
      </Box>
    </Box>
  )
}

export default SortieDirectoryPage
